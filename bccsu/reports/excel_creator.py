...
class ReportCreator:
    def __init__(self, filename=None, request_path=None, template=None, *args, **kwargs):
        ...
        self.wb = load_workbook(filename, template=template, *args, **kwargs)
    ...
    def set_page_title(self, title, pos=None):
        set_page_title(self.ws, title, pos=pos)

    def add_note(self, note, position=None, merge=None):
        add_note(self.ws, note, position=position, merge=merge)

    def add_title(self, title, position=None):
        add_title(self.ws, title, position=position)

    def write_table(self, df, table_start_pos=None, write_index=True, write_headers=True, title=None, percent=False):
        return write_table(self.ws, df, table_start_pos=table_start_pos, write_index=write_index,
                           write_headers=write_headers, title=title, percent=percent)

    def insert_image(self, image_path, pos):
        insert_image(self.ws, image_path, pos)
